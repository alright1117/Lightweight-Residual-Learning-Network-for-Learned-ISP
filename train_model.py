...
with tf.Graph().as_default(), tf.compat.v1.Session(config=config) as sess:
    ...
    if arch == "model":
        enhanced = model(phone_, instance_norm=inst_norm, instance_norm_level_1=False, num_maps_base=num_maps_base)
    ...
    loss_content = tf.nn.l2_loss(enhanced_vgg[CONTENT_LAYER] - dslr_vgg[CONTENT_LAYER]) / content_size / 100

    # Final loss function
    loss_generator = loss_mse * 20 + (1 - loss_ssim) * 20
    ...
    # saver.restore(sess, "models/original/punet_pretrained.ckpt")
    ...
    train_data, train_answ = load_train_patch(dataset_dir, dslr_dir, phone_dir, PATCH_WIDTH, PATCH_HEIGHT, DSLR_SCALE)
    ...
    num_train_iters = restore_iter+num_train_iters if restore_iter > 0 else num_train_iters
