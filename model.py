...
def model(input, instance_norm=False, instance_norm_level_1=False, num_maps_base=16):
    ...
    with tf.compat.v1.variable_scope("generator"):
        ...
        conv_l5_d1 = LFB(pool4, num_maps=num_maps_base*8, instance_norm=instance_norm)
        conv_l5_d2 = LFB(conv_l5_d1, num_maps=num_maps_base*8, instance_norm=instance_norm)

        conv_t4b = _conv_tranpose_layer(conv_l5_d2, num_maps_base*4, 3, 2)      # 8 -> 16

        # -----------------------------------------
        # Processing: Level 4,  Input size: 16 x 16
        conv_l4_d6 = conv_l4_d1
        conv_l4_d7 = stack(conv_l4_d6, conv_t4b)
        conv_l4_d8 = _conv_multi_block(conv_l4_d7, 3, num_maps=num_maps_base*4, instance_norm=instance_norm)

        conv_t3b = _conv_tranpose_layer(conv_l4_d8, num_maps_base*2, 3, 2)      # 16 -> 32

        # -----------------------------------------
        # Processing: Level 3,  Input size: 32 x 32
        conv_l3_d6 = conv_l3_d1
        conv_l3_d7 = stack(conv_l3_d6, conv_t3b)
        conv_l3_d8 = _conv_multi_block(conv_l3_d7, 3, num_maps=num_maps_base*2, instance_norm=instance_norm)

        conv_t2b = _conv_tranpose_layer(conv_l3_d8, num_maps_base, 3, 2)       # 32 -> 64

        # -------------------------------------------
        # Processing: Level 2,  Input size: 64 x 64
        conv_l2_d7 = conv_l2_d1
        conv_l2_d8 = stack(_conv_multi_block(conv_l2_d7, 3, num_maps=num_maps_base, instance_norm=instance_norm), conv_t2b)
        conv_l2_d9 = _conv_multi_block(conv_l2_d8, 3, num_maps=num_maps_base, instance_norm=instance_norm)
        ...
        conv_l0 = _conv_tranpose_layer(conv_l1_d12, num_maps_base//4, 3, 2)
...
def _conv_multi_block(input, max_size, num_maps, instance_norm, ratio=8, attention=False):
...
def AWRU(x, num_maps, instance_norm=False):
    res_scale = tf.Variable(tf.ones(1))
    x_scale = tf.Variable(tf.ones(1))
    
    body = _conv_layer(x, num_maps, 3, 1, relu=True, instance_norm=instance_norm)
    body = _conv_layer(body, num_maps, 3, 1, relu=False, instance_norm=instance_norm)
    output = res_scale * body + x_scale * x
    
    return output

def LFB(x, num_maps, instance_norm):
    res_scale = tf.Variable(tf.ones(1))
    x_scale = tf.Variable(tf.ones(1))
    
    x1 = AWRU(x, num_maps, instance_norm=False)
    x2 = AWRU(x1, num_maps, instance_norm=False)
    x3 = AWRU(x2, num_maps, instance_norm=False)
    x4 = AWRU(x3, num_maps, instance_norm=False)
    
    res = _conv_layer(tf.concat([x1, x2, x3, x4], 3), num_maps, 3, 1, relu=False)
    
    return res_scale * res + x_scale * x
